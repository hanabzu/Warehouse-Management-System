...
| 어드민 | 필요한 브랜드의 상품을 요청하거나 처리할 수 있다.   |                 | <- 어드민이 창고/점주에게 요청을 받은 브랜드를 해당 브랜드에게 신청하는 것이라고 이해했는데 이는 이 어플리케이션에서 필요한 기능은 아니라고 생각합니다. 
| 어드민 | 상품이 부족할 시 대체할 수 있게 창고를 확인 할 수 있다.     |                 | <-이해 어려움. 창고/지점에 상품이 부족? 자동 발주 기능을 위한 것인가요?
| 어드민 | 해당 창고의 제품 수량을 실시간으로 확인 할 수 있다.       | REQ 어드민 파트 먼저 수정 보완 필요 (가입수락기능 등) |
| 어드민 | 배송 상황을 확인 할 수 있다.     |                 | <- 배송 상황 확인이 필요한가? 에 대해 생각해 볼 필요가 있을 것 같아요.
| 어드민 | 리포트를 통해 물품에 대한 구매량을 확인 할 수 있다        |                 | <- 어떤 물품 구매량인지 모르겠어요. 창고에 입고하는 걸 말하는 건가요?
